...
void bfs(int root) {    // 시작 : 1
    int level_num = 1;
    row_height[root][1] = 1; // 1 (root node)의 level = 1
...
int main() {
    ...
    /* root node가 1이 아닐 수 있다!! */
    // root node 찾기 (parent가 없음)
    int root_node = 1;
    for (int i = 1; i <= N; i++) {
        if (arr[i][0] == 0) {
            root_node = i;
    ...
    for (int i = 1; i <= N; i++) {
        ...
        if (width == max_width) {
            if (level < level_idx) { level_idx = level; }
        }
    }
